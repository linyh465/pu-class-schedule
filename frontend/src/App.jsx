...
const COURSE_MAP = new Map(ALL_COURSES.map(course => [course.id, course]));
...
const loadSavedCourses = () => {
  try {
    ...
    return saved.map(id => COURSE_MAP.get(id)).filter(Boolean);
  } catch {
    return [];
  }
};
...
export default function App() {
  const [selectedCourses, setSelectedCourses] = useState(loadSavedCourses);
  ...
  const handleAIAnalysis = async () => {
    ...
    const courseList = selectedCourses
      .map(c => `- [${c.id}] ${c.name} (${c.type}, ${c.credits}學分, 星期${c.times[0].day} 第${c.times[0].periods.join(',')}節)`)
      .join('\n');
